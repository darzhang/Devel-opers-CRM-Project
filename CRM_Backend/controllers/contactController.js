...
const createContact = async (req, res) => {
  ...
  if (department !== null) {
    ...
    // it doesn't exist so create a new department with that name and save it to the database
  ...
  const organisation = await Organisation.findOne({
    nameLower: organisationName.toLowerCase(),
  }).lean();
  let organisationId = "";
  // if it exist, just take the id
  if (organisation !== null) {
    ...
    // it doesn't exist so create a new organisation with that name and save it to the database
...
const editContact = async (req, res) => {
  ...
  try {
    ...
    if (!contact) {
      // if contact is not already in database, return an error
      res.status(400);
...
const deleteContact = async (req, res) => {
  try {
    // delete contact based on the given id
