...
const AddEvent = ({ event, onEdit, readOnly }) => {
  ...
  const [startTime, setStartTime] = useState(
    moment(new Date(event.startTime)).format("yyyy-MM-DDTHH:mm")
  );
  const [endTime, setEndTime] = useState(
    moment(new Date(event.endTime)).format("yyyy-MM-DDTHH:mm")
  );
  ...
  return (
    <form className="add-form" onSubmit={onSubmit}>
      <div
        style={{ display: readOnly === false ? "block" : "none" }}
      ...
      {!readOnly && (
        <div>
          ...
          <Button
            ...
            color="secondary"
            style={buttonStyle}
            onClick={() => (window.location = `/event/${event._id}`)}
          >
            Cancel
          </Button>
        </div>
