...
export default function ContactProfile(props) {
  ...
  useEffect(() => {
    ...
    const getContact = async () => {
      ...
      await axios
        .get(BASE_URL + "/contact/" + id, { withCredentials: true })
        .then((res) => {
          const data = res.data;
          setValues({
  ...
  const getDepartmentName = async (departmentId) => {
    const BASE_URL = "http://localhost:8080";
    await axios
      .get(BASE_URL + "/department", { withCredentials: true })
      .then((res) => {
  ...
  const getOrganisationName = async (organisationId) => {
    const BASE_URL = "http://localhost:8080";
    await axios
      .get(BASE_URL + "/organisation", { withCredentials: true })
      .then((res) => {
  ...
  const deleteContact = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios
          .delete(BASE_URL + "/contact/" + id, { withCredentials: true })
          .then(() => {
            Swal.fire({
              title: "Success!",
  ...
  const onSubmit = (e) => {
    ...
    axios
      .post(
        url,
        { _id, ...values, departmentName, organisationName },
