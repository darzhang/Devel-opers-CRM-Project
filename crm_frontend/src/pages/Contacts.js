...
export default function Contacts() {
  ...
  const getContacts = async () => {
    ...
    await axios
      .get(BASE_URL + "/contact", { withCredentials: true })
      .then((res) => {
        const list = res.data;
        ...
        sortedList.forEach(async (contact) => {
          ...
          const department = deps.filter(
            (department) => department._id === contact.departmentId
          );
          contact.departmentName = department[0].departmentName;
          const organisation = orgs.filter(
            (organisation) => organisation._id === contact.organisationId
          );
  ...
  const showDetailColumn = {
    ...
    renderCell: (contact) => {
      return (
        <a
          href={"/contact/profile/" + contact.id}
        ...
        {!isLoading && (
          <div>
            ...
            <div style={buttonDivStyle}>
              <Button
                variant="contained"
                style={{ textTransform: "none", marginRight: "1%" }}
