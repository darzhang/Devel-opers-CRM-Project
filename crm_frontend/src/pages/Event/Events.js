...
const Events = () => {
    ...
    renderCell: (cellValues) => {
      return (
        <Tooltip title="Delete Event">
          <IconButton
            onClick={() =>
              deleteEvent(cellValues.row.id, cellValues.row.eventName)
            }
          >
            <DeleteIcon />
          </IconButton>
  ...
  return (
    <div className="Events" style={{ marginTop: "2%", marginLeft: "75px" }}>
      <div>
        <h1>{isUpcoming ? "Events" : "Past Events"}</h1>
      </div>
      {!isLoading && (
      ...
      <div className="listOfEvents" style={{ marginTop: "1.5%" }}>
        {!isLoading ? (
          isUpcoming ? (
            ...
            <DataGridComp events={pastEvents} columns={columns}></DataGridComp>
          )
        ) : (
          <CircularProgress />
        )}
